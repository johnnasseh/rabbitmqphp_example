...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        try {
            ...
            $mail->Body = "Hello $username, <br> Don't miss your liked event: <strong>$eventName</strong> happening on <strong>$eventDate</strong> at <strong>$eventVenue</strong>.";

            $mail->send();
            echo json_encode(["status" => "success", "message" => "Email sent successfully"]);
        } catch (Exception $e) {
            error_log("Mailer Error: {$mail->ErrorInfo}");
            echo json_encode(["status" => "fail", "message" => "Email could not be sent. Mailer Error: {$mail->ErrorInfo}"]);
        }
    } catch (Exception $e) {
