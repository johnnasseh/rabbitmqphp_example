...
function registerBundle($bundleName, $version, $status, $path) {
    $db = getDeployDB();
    $query = $db->prepare("INSERT INTO Bundles (bundle_name, version, status, path) VALUES (?, ?, ?, ?)");
...
function fetchLatestBundle($bundleName, $status = 'new') {
    $db = getDeployDB();
    $query = $db->prepare("SELECT bundle_name, version, path FROM Bundles WHERE bundle_name = ? AND status = ? ORDER BY version DESC LIMIT 1");
...
function fetchPreviousPassedBundle($bundleName) {
    $db = getDeployDB();
    $query = $db->prepare("SELECT bundle_name, version, path FROM Bundles WHERE bundle_name = ? AND status = 'passed' ORDER BY version DESC LIMIT 1 OFFSET 1");
    $query->bind_param('s', $bundleName);
    $query->execute();
    $result = $query->get_result();
    if ($row = $result->fetch_assoc()) {
...
function updateBundleStatus($bundleName, $version, $status) {
    $db = getDeployDB();
    $query = $db->prepare("UPDATE Bundles SET status = ? WHERE bundle_name = ? AND version = ?");
